...
static void clear_account_data() {
    NSString *guestFile = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/com.garena.msdk/guest.dat"];
    [[NSFileManager defaultManager] removeItemAtPath:guestFile error:nil];
    SecItemDelete((__bridge CFDictionaryRef)@{(__bridge id)kSecClass: (__bridge id)kSecClassGenericPassword});
}

__attribute__((constructor))
static void init_v16() {
    clear_account_data();
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(10 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        UIWindow *win = [[UIApplication sharedApplication] keyWindow];
        UILongPressGestureRecognizer *seg = [[UILongPressGestureRecognizer alloc] initWithTarget:[RickzzMenu class] action:@selector(open:)];
        seg.numberOfTouchesRequired = 3; seg.minimumPressDuration = 2.0;
        [win addGestureRecognizer:seg];
    });
}

+ (void)open:(UILongPressGestureRecognizer *)g { if (g.state == UIGestureRecognizerStateBegan) [self toggle]; }
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _container = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 450, 270)];
        _container.center = self.center;
        _container.backgroundColor = [UIColor colorWithWhite:0.0 alpha:0.98];
        _container.layer.cornerRadius = 20; _container.layer.borderWidth = 1.5;
        ...
        _fovSlider = [[UISlider alloc] initWithFrame:CGRectMake(50, 225, 350, 30)];
...
- (void)mudouSwitch:(UISwitch *)sender {
    NSString *name = self.itens[sender.tag];
    if ([name isEqualToString:@"Aimbot Pro"]) aim_active = sender.on;
    if ([name containsString:@"ESP"] || [name isEqualToString:@"Tracer"]) esp_active = sender.on;
    if ([name isEqualToString:@"Cabeça"] && sender.on) { aim_target = 0; [self off:6]; }
    if ([name isEqualToString:@"Peito"] && sender.on) { aim_target = 1; [self off:5]; }
    if ([name isEqualToString:@"ATIVAR FOV"]) {
